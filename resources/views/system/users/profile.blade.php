...
@section('page_title')
    <x-header.title name="المستخدمين">
        <x-header.breadcrumb-item :href="route('system.users.index')" name="المستخدمين"/>
        <x-header.breadcrumb-item name="تعديل بياناتي"/>

    </x-header.title>
@endsection
@section('content')
    <form id="editForm" class="form" action="{{route('system.users.do.profile')}}" method="post">
        @csrf
    <x-theme.card>
    ...
    <x-slot name="toolbar">
        ...
        <div class="row">
            ...
            <div class="col-md-3 d-flex flex-wrap justify-content-center">
                <x-inputs.image name="image" :value="$out->image" title="الصورة" width="300" height="300"/>
                <x-theme.button type="href" :href="route('system.users.profile.password')">
