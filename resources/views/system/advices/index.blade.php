...
@section('title',lng('dashboard.advices.advices','نصائح للحوامل'))
@section('page_title')
    <x-header.title :name="lng('dashboard.general.dashboard','لوحة التحكم')">
        <x-header.breadcrumb-item :name="lng('dashboard.advices.advices','نصائح للحوامل')"/>
    </x-header.title>
@endsection
@push('css')
...
@section('search_content')
    <x-search :searchable="\App\Models\Advice::getSearchable()"/>
@endsection
@section('content')
    <x-theme.card>
        <x-slot name="toolbar">
            <x-theme.button type="openmodal"  class="btn btn-sm btn-flex btn-light-primary mx-2"
                            :modaltitle="lng('dashboard.advices.add_service_title','اضافة نص جديد')"
                            modalsize="modal-xl"
                            modallevel="2"
                href="{{route('system.advices.create')}}">
        ...
        <x-slot name="bulkactions">
            <button type="button" class="btn btn-danger mx-2 btn-bulk-action"
                    data-url="{{route('system.advices.delete')}}" data-token="{{csrf_token()}}">حذف المحدد
            </button>
            <button type="button" class="btn btn-success mx-2 btn-bulk-action"
                    data-url="{{route('system.advices.activate')}}" data-token="{{csrf_token()}}">تفعيل المحدد
            </button>
            <button type="button" class="btn btn-warning mx-2 btn-bulk-action"
                    data-url="{{route('system.advices.deactivate')}}" data-token="{{csrf_token()}}">تعطيل المحدد
            </button>
        </x-slot>
        <x-theme.tables.table>
            <x-slot name="thead">
                ...
                <th class="w-500px">رابط اليوتيوب</th>
